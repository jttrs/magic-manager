...
| Starter Collection | (card pool) | `sealedProduct` "Foundations Starter Collection" → ONE `StarterCollection_FDN` deck (387 cards, 278 distinct) — a build-your-own LIBRARY, not a playable deck, so it ingests as **deconstructed** (auto-classified by name + card_count>150: cards loose in inventory, marker deck row). Also 3 random Play Boosters (not importable). Card-level `startercollection` tag on 278 `fdn` cards. No separate Scryfall set code. `mm deck add-precon fdn "Starter Collection"` auto-routes to deconstructed; flip to `built` if keeping it intact. |
