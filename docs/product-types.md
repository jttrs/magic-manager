...
> **Ingest state — built / deconstructed.** Every imported precon copy is a `decks`
> row carrying its MTGJSON fileName and a `precon_state` (V11): **built** (assembled deck,
> cards pledged) or **deconstructed** (a deck torn down for parts — recipe kept, cards loose).
> Unit counts derive from these rows (`decks.precon_unit_counts`). `mtgjson.default_precon_state(fileName)`
> picks the default — **deconstructed** for products with no real decklist (name in `POOL_NAME_PATTERNS`,
> a Scene Box's component decks, or a single deck with card_count > `POOL_CARD_COUNT_THRESHOLD`=150),
> else **built**; a user can flip a pool-shaped product to `built` manually when keeping a copy intact.
> Override per-ingest with `mm deck add-precon -c/-d` or `mm deck import-precon --state`.
...
| **Scene Box** | `Box Set`-type deck of "scene" cards; `sealedProduct` name contains "Scene Box" | Fixed set of scene cards; **product-EXCLUSIVE** (`booster: null`, not pack-pullable) | **deconstructed** — never a playable deck; `default_precon_state` auto-classifies (name match) so `mm deck add-precon`/`import-precon` route it to deconstructed. User can flip to `built` manually if keeping a copy intact. |
| **Starter Collection** | `sealedProduct` name "… Starter Collection", category `box_set`/subtype `starter_deck`; ONE large `contents.deck` (387 cards for FDN) | Build-your-own card LIBRARY, not a deck (+ 3 Play Boosters, random, not imported) | **deconstructed** — auto-classified by name + card_count > 150 (cards loose in inventory, marker deck row). User can flip to `built` if keeping a copy intact. |
...
- **Handling: deconstructed.** A Scene Box is loose collectible cards, never a playable deck,
  so it ingests as state `deconstructed` (cards → inventory, a marker deck row, not pledged).
  `default_precon_state` auto-classifies it via the "Scene Box" product name, so
  `mm deck add-precon` / `import-precon` route it to deconstructed with no flag. The user can
  flip a copy to `built` manually with `mm deck add-precon <name> -c` if keeping the box intact.
