...
V2 fact tables: `cards`, `inventory`, `wishlist_entries`, `decks`, `deck_cards`, `ingest_log`, `set_targets`, `precon_ledger` (SCHEMA_V7; built/torn-down precon-unit counts). Pre-V2 used a single conflated `list_rows` table; see `docs/pre-v2-inventory-snapshot.md` for the migration baseline.
...
- `inventory.py`, `wishlist.py`, `decks.py` — V2 fact-table CRUD + value rollups. `decks.py` also holds `import_precon` and the `precon_ledger` CRUD.
