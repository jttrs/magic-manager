...
- Cataloging a new set from scratch (use [[generate-set-checklist]] to produce the inventory checklist XLSX first).
...
- [[generate-set-checklist]] — when the user wants the inventory-checklist XLSX for a whole set (with every printing as a row). That's the opposite direction — `master-list` writes the universe (default `--mode add`, blank qty cells; `--mode modify` for current-inventory snapshots); `mm query xlsx 'set:X missing'` writes the gap report.
