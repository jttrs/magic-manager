...
| "What am I missing from set X?" / "what's left to buy from X?" / "missing rare/mythics from X?" / "give me a checklist of what I need from X" | Use the [[missing-from-set]] skill — `uv run mm query missing-set <CODE>`. Single command, set-agnostic. Emits Scryfall URL chunks in chat (printing-specific, cheapest-first) plus two `file://` links: a **missing checklist** XLSX and a ManaPool bulk-add MD. The artifacts are NEVER rendered inline; only the URL table goes in chat. **Note**: the missing checklist is distinct from the **inventory checklist** produced by [[generate-set-list]] — different column shape, different filter rules, different `_meta.kind` value. See either skill's "Not to be confused with" section. |
| "What's available to put in a new deck?" / "What's NOT committed to an existing deck?" | `uv run mm query show 'inventory available'`. Subtracts the SUM of `deck_cards.count` per (printing, finish) from the inventory quantity. Owning 2 Sol Rings with both committed → 0 available. |
| "Where is my Sol Ring?" / "Which decks contain X?" / "Find a card by name or set+CN" | Use [[import-precon]] cross-reference: `uv run mm deck find <query>` where query is a scryfall_id, a quoted `'set cn'` pair, or an exact card name. Reports the per-deck commitments plus the owned/committed/available math. |
| "I bought N copies of <precon>" / "I just opened a Commander deck" / "Import the contents of <precon>" | Use the [[import-precon]] skill. Routes to `mm deck import-precon` which creates N named decks AND adds cards to inventory in one shot, using MTGJSON for the precon contents. Default keeps the precons assembled; `--deconstruct` opt-in for breaking them down. |
...
| `available` | Subtract deck commitments from inventory quantities. **Only applies to the `inventory` term.** Result: `inventory.qty - SUM(deck_cards.count)` per (printing, finish), with rows where the difference is ≤0 dropped. Over-committed cards (more decked than owned) emit a stderr warning so the inconsistency is visible. Use to answer "what's free for a new deck?" | `inventory available` |
...
| `treatment=regular\|alt\|collectible-alt\|preferred\|any-alt\|b\|fa\|shw\|ext\|sm\|ff` | Filter by treatment class (computed from `frame_effects`/`full_art`/`promo_types` via `treatments.compute_treatment`). `regular` = no codes; `alt` = non-empty AND no `ext`; `collectible-alt` = `alt` minus pure-`{ff}`; `preferred` = `collectible-alt` minus datestamped-with-sibling minus family-configured fancy-foil-dupes (e.g. surgefoil duplicates of borderless prints in FIN); `any-alt` = any non-empty (includes `ext`); individual codes match exact code presence. **`preferred` requires per-family config in `selectors.FAMILY_DUPE_FOIL_PROMO_TYPES`** — uses the family's anchor code to look up which `promo_types` signal "same art on a fancy-foil sheet" vs "unique art that happens to come on a fancy-foil sheet." Errors out clearly on unconfigured families. See `treatments.LEGEND` for what each code means. The `[[missing-from-set]]` skill uses `preferred` as the default. | `set:fin+related missing treatment=preferred` |
...
- [[generate-set-list]] — when the user wants the inventory-checklist XLSX for a whole set (with every printing as a row). That's the opposite direction — `master-list` writes the universe (default `--mode add`, blank qty cells; `--mode modify` for current-inventory snapshots); `mm query xlsx 'set:X missing'` writes the gap report.
