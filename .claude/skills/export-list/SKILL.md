...
- **`set:<code> missing` requires the set to have been synced and registered.** `set:` reads from the local cards table, so the set's printings must have been pulled via `mm set sync` or `mm set master-list`. If the selector returns 0 rows but the user expects results, run [[generate-set-checklist]] for that set first.
