...
- **Only inventory checklists (`_meta.kind: "inventory"`) are ingestable.** "Missing checklists" produced by `mm query missing-set` carry `_meta.kind: "missing"` and have a different column shape (no `qty_normal`/`qty_foil`) — `mm set ingest` will fail (or produce nonsense) if pointed at one. If a user moves a missing checklist into `checklists/` and tries to ingest it, surface the column-shape mismatch as the diagnostic; don't try to coerce it. The two artifacts have different purposes: inventory = "catalog what I own," missing = "shopping list of what I don't." See [[generate-set-checklist]] and [[missing-from-set]] for the distinction.
